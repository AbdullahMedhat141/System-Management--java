...
public class AttendanceController implements Initializable {
    ...
    public void add() {
        try {
            if (model.getLogicStudentById(Integer.parseInt(studentId.getText())) == null) {
    ...
    public void remove(ActionEvent event) throws IOException {
        ...
        model2.deleteOneStudentById(selected.getId());
        model2.save_allStudents(s);
    }
    public void delete(ActionEvent event) throws IOException {
        Date selected = datesTable.getSelectionModel().getSelectedItem();
        datesTable.getItems().remove(selected);
        model2.deleteDateByName(selected.getDate());
        model2.save_Dates();
    }
}
